...
> 一个功能完整的群聊积分系统，支持签到、抽奖、兑换、排行、生日、口令等多项功能。当前版本 **v0.3.1**。
...
- **数据库版本**: 当前 schema v3（积分一号跨群共享：`accounts` 表按 QQ 全局唯一账户，`users` 仅存群级数据），旧库首次加载时自动迁移（v2 → v3 多群余额取 MAX 合并；v1 → v2 负分头衔原名片、流水操作人字段），升级前建议先备份数据库
...
插件内置零依赖测试套件（**151 项**功能/并发/安全/稳定性/压力测试 + 性能基准，覆盖 18 个套件：功能正确性、并发竞态、安全、性能、稳定性、彩蛋/日期奖励、生日、活跃奖励、管理指令、handler 层、服务/DAO、工具/配置、限流器、备份恢复、迁移、压力/浸泡/随机化、指令图、跨群共享），全部在临时库上运行，不触碰生产数据：
