...
> 一个功能完整的群聊积分系统，支持签到、抽奖、兑换、排行、生日、口令、打劫等多项功能。当前版本 **v0.4.0**。
...
- 打劫：无前缀「打劫 @目标」抢群友积分（成功抢得部分积分、失败扣成本；成功率/收益曲线/门槛/冷却/每日上限均可配置；目标可被抢成负分并联动负分头衔）

**积分消费**

- 个人抽奖：口令验证，五档权重概率（特等奖/一等奖/二等奖/三等奖/参与奖），每日限次按 QQ 全局统计（跨群共享）；反馈含**消耗 / 积分变化 / 当前积分**
- 兑换玩法：库存原子扣减，兑换记录核销/驳回三态（通过/驳回可互切，**驳回退回积分并恢复库存**），**核销/驳回后群内 @ 通知兑换者**（发送失败发警告）；成功反馈含**订单号 / 剩余库存 / 积分余额**与"联系管理员核销"提示
...
| 有前缀 | `/兑换` | 查看可兑换物品 |
| 有前缀 | `/兑换 <物品ID> [数量]` | 兑换物品（成功反馈含订单号/剩余库存/积分余额与核销提示） |
...
| `/核销 [通过\|驳回] <记录编号> [备注]` | 核销/驳回兑换订单并 @ 通知兑换者（未写动作默认通过；驳回退回积分并恢复库存；通过/驳回可互切） |
...
插件内置零依赖测试套件（**184 项**功能/并发/安全/稳定性/压力测试 + 性能基准，覆盖 19 个套件：功能正确性、并发竞态、安全、性能、稳定性、彩蛋/日期奖励、生日、活跃奖励、管理指令、handler 层、服务/DAO、工具/配置、限流器、备份恢复、迁移、压力/浸泡/随机化、指令图、跨群共享、打劫系统），全部在临时库上运行，不触碰生产数据：
