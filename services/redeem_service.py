...
class RedeemService:
    ...
    async def redeem(self, qq: str, group_id: str, item_id: int, quantity: int = 1) -> dict:
        ...
        async def _tx(conn):
            cur = await conn.execute("UPDATE redeem_items SET stock=stock-? WHERE id=? AND (stock=-1 OR stock>=?)", (quantity, item_id, quantity))
            if cur.rowcount == 0:
                raise ValueError("\u5e93\u5b58\u4e0d\u8db3")
            await conn.execute("UPDATE users SET points=points-? WHERE qq=? AND group_id=?", (total_cost, qq, group_id))
            ...
            record_no = await generate_record_no(self._dao)
