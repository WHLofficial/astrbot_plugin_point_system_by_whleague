...
class PointSystemPlugin(Star):
            ...
            try:
                ...
                if result.get("announced"):
                    ...
                    try:
                        chain = (
                            MessageChain()
                            .message("\U0001f382 \u751f\u65e5\u795d\u798f\uff01\u4eca\u5929\u8fc7\u751f\u65e5\u7684\u670b\u53cb\u6709\uff1a")
                        )
                        for u in users:
                            chain.at(str(u), u)
                        chain.message("\n\u795d\u5927\u5bb6\u751f\u65e5\u5feb\u4e50\uff01")
                        await self.context.send_message(origin, chain)
                        await self.dao.mark_birthday_announced(gid, today, json.dumps(users))
                    except Exception as e:
    ...
    @filter.command("\u5151\u6362\u8bb0\u5f55")
    async def cmd_redeem_records(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        msg = event.get_message_str()
    ...
    @filter.command("\u6838\u9500")
    async def cmd_verify(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        msg = event.get_message_str()
        parts = msg.split(maxsplit=2)
        if len(parts) < 2:
    ...
    @filter.command("\u6dfb\u52a0\u5151\u6362")
    async def cmd_add_item(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.add_item(event):
            yield r

    @filter.command("\u5220\u9664\u5151\u6362")
    async def cmd_delete_item(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.delete_item(event):
            yield r

    @filter.command("\u4fee\u6539\u5151\u6362")
    async def cmd_modify_item(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
    ...
    @filter.command("\u52a0\u5206")
    async def cmd_add_points(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.adjust_points(event, "\u52a0\u5206"):
            yield r

    @filter.command("\u6263\u5206")
    async def cmd_sub_points(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.adjust_points(event, "\u6263\u5206"):
            yield r

    @filter.command("\u8bbe\u7f6e\u4eca\u65e5\u53e3\u4ee4")
    async def cmd_set_daily_kw(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.set_daily_kw(event):
            yield r

    @filter.command("\u6e05\u9664\u4eca\u65e5\u53e3\u4ee4")
    async def cmd_clear_daily_kw(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
    ...
    @filter.command("\u8bbe\u7f6e\u6298\u6263")
    async def cmd_set_discount(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.set_discount(event):
            yield r

    @filter.command("\u6e05\u9664\u6298\u6263")
    async def cmd_clear_discount(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
    ...
    async def cmd_add_date_reward(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.admin_handler.add_date_reward(event):
            yield r

    @filter.command("\u5220\u9664\u65e5\u671f\u5956\u52b1")
    async def cmd_delete_date_reward(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
    ...
    @filter.command("\u8bbe\u7f6e\u751f\u65e5")
    async def cmd_set_birthday(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.birthday_handler.set_birthday(event):
            yield r

    @filter.command("\u67e5\u751f\u65e5")
    async def cmd_query_birthday(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
    ...
    @filter.command("\u7b7e\u5230\u7edf\u8ba1")
    async def cmd_signin_stats(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        async for r in self.ranking_handler.stats(event):
            yield r

    @filter.command("\u6d41\u6c34")
    async def cmd_transactions(self, event: AstrMessageEvent) -> AsyncGenerator[MessageEventResult, None]:
        ...
        if len(parts) >= 2:
            if parts[1] == "all":
                ...
                if parsed_target is not None:
                    ...
                    yield event.plain_result("\u53c2\u6570\u9519\u8bef: \u7528\u6cd5 / \u6d41\u6c34 [\u9875\u7801 | all | @\u7528\u6237] [\u9875\u7801]")
                    return
