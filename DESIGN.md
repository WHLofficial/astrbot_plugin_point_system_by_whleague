...
| 9 | **兑换玩法** | 积分换物品，库存管理（原子扣减），兑换记录核销/驳回三态（pending/verified/rejected 可互切，驳回退回积分并恢复库存），核销/驳回后通知兑换者（默认原兑换群 @，可配置私信；全局管理员可跨群核销），限时折扣 |
...
驳回通知文案：`❌ 你的兑换订单 {no}（{item} x{qty}）已被驳回，消耗的 {cost} 积分已退回（理由：{原因}）`；
...
| 有前缀 | `/核销 [通过\|驳回] <记录编号> [备注]` | 核销/驳回兑换订单并通知兑换者（未写动作默认通过，驳回退回积分恢复库存；全局管理员可跨群核销，渠道见 `redeem_notify_channel`） | 管理员 |
