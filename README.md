...
> 一个功能完整的群聊积分系统，支持签到、抽奖、兑换、排行、生日、口令、打劫等多项功能。当前版本 **v0.5.2**。
...
- 兑换玩法：库存原子扣减，兑换记录核销/驳回三态（通过/驳回可互切，**驳回退回积分并恢复库存**），**核销/驳回后通知兑换者**（默认原兑换群 @通知，可配置为私信；全局管理员可跨群核销；发送失败发警告）；成功反馈含**订单号 / 剩余库存 / 积分余额**与"联系管理员核销"提示
...
| `/设置 <配置项> <值>` | 修改运行时配置 |
| `/查看配置` | 查看当前配置 |
| `/兑换记录 all/全部 [页码]` | 查看全部兑换记录 |
| `/兑换记录 pending/未核销 [页码]` | 查看未核销记录 |
