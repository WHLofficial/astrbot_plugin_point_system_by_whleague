...
> 一个功能完整的群聊积分系统，支持签到、抽奖、兑换、排行、生日、口令等多项功能。当前版本 **v0.1.2**。
...
插件内置零依赖测试套件（**118 项**功能/并发/安全/稳定性/压力测试 + 性能基准，覆盖 16 个套件：功能正确性、并发竞态、安全、性能、稳定性、彩蛋/日期奖励、生日、活跃奖励、管理指令、handler 层、服务/DAO、工具/配置、限流器、备份恢复、迁移、压力/浸泡/随机化），全部在临时库上运行，不触碰生产数据：
