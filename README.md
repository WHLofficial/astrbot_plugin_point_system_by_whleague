...
- 数据清空：`/清空数据`（本群，清零本群成员共享积分与群级记录，积分全局共享故其他群同步归零；发言统计一并清除）与 `/清空全部数据`（全局），验证码二次确认，清空前自动备份并恢复负分头衔名片
...
| `/删除日期奖励 <ID>` | 软删除日期奖励 |
| `/查看日期奖励` | 查看日期奖励列表 |
| `/清空数据` | 清空本群成员的共享积分与群级记录（含发言统计；积分全局共享，其他群余额同步归零；成员关系保留），需验证码二次确认，清空前自动备份 |
| `/清空全部数据` | 清空全部数据（含商店、日期奖励、管理名单、发言统计），仅 AstrBot 全局管理员，需验证码二次确认，清空前自动备份 |
...
插件内置零依赖测试套件（**239 项**功能/并发/安全/稳定性/压力测试 + 性能基准，覆盖 22 个套件：功能正确性、并发竞态、安全、性能、稳定性、彩蛋/日期奖励、生日、活跃奖励、管理指令、handler 层、服务/DAO、工具/配置、限流器、备份恢复、迁移、压力/浸泡/随机化、指令图、跨群共享、打劫系统、竞猜同步、AstrBot 4.28.0 适配守卫、发言统计），全部在临时库上运行，不触碰生产数据：
...
`s22_speak_stat` 覆盖发言统计全链路：计数命中（文本/@/纯图片）与空消息、非群、机器人自身排除，同日 UPSERT 只保留一行并累加，业务日边界（03:59 与 04:01 落不同日期），今日/本周/本月/上月/累计五档区间口径（含跨月样本），本群排名与同分并列、占比与百分位，称号七档阈值边界与配置覆盖及非法回退，计数写库抛异常时活跃奖励仍执行，计数与活跃奖励的双入口拆分，跨群隔离，触发词严格匹配，以及旧库（`schema_version=5`）启动即建表并升级到 6；`/清空数据` 与 `/清空全部数据` 一并清除发言统计（分别由 s1 与 s9 覆盖）。
