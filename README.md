...
| 负分联动 | 负分仅可签到恢复积分，不能抽奖/兑换/活跃奖励，自动将群名片改为"群女仆X号"头衔，回正后恢复原名片 |
| 自动备份 | 多本地目标路径，定时备份（默认每天 04:00，可配置），备份前 WAL checkpoint |
...
> 每日刷新时刻（`signin_refresh_time`）统一作用于签到、连签、每日首签、每日口令、抽奖次数等全部每日逻辑；一天区间为该时刻至次日同一时刻。
