...
插件内置零依赖测试套件（**191 项**功能/并发/安全/稳定性/压力测试 + 性能基准，覆盖 19 个套件：功能正确性、并发竞态、安全、性能、稳定性、彩蛋/日期奖励、生日、活跃奖励、管理指令、handler 层、服务/DAO、工具/配置、限流器、备份恢复、迁移、压力/浸泡/随机化、指令图、跨群共享、打劫系统），全部在临时库上运行，不触碰生产数据：
